...
# move e at the center to more left
e[:, :, 50:68] = np.roll(e[:, :, 50:68], -6, axis=1)
...
kernel = np.linspace(0.8, 1, e.shape[2]) ** 2
kernel = kernel[::-1]
kernel /= kernel[-1]
...
# multiply a fraction of xy plane
...
xc2, yc2, zc2 = 450., 520., 80
sigma2 = 250.
kernel5 = 1. * np.exp(- ((x-xc2)**2+(y-yc2)**2)/ (2. * sigma2**2))
...
for i in range(e.shape[2]):
    ...
    if i < 100:
        ...
        if i > 40 and i < 74:
            fig, ax, cc = graph.color_plot(data['y'][..., i], data['x'][..., i], e[..., i] / kernel[i] * kernel4 * kernel5, vmin=0,
                                           vmax=10 * 10 ** 3)
            e_mod[..., i] = e[..., i] / kernel[i] * kernel4 * kernel5
