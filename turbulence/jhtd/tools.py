...
"""
Modules to analyze the JHTD data
author: takumi
"""
...
def compute_lambda(filepath, savedir):
    """
    ...
    save_computed_results(U2ave, Ux2ave, Uy2ave, dUidxi2ave, dUxdx2ave, dUydy2ave, savedir, save=True)
    save_computed_results_Local(lambdaT_Local, Re_lambdaT_Local, lambdaTx_Local, Re_lambdaTx_Local, lambdaTy_Local, Re_lambdaTy_Local, savedir,save=True)
...
def plot_lambda_and_Relambda(filepath, lambdaT, Re_lambdaT, lambdaTx, Re_lambdaTx, lambdaTy, Re_lambdaTy, savedir, save=False):
    ...
    plt.yticks(size=50)
    if save:
        filename = 'lambdauyLocal_z=0_t=0_ave{0:.3f}'.format(lambdaTyave)
        filepath = savedir + filename
        graph.save(filepath, ext='png')
    ...
    cbar2=plt.colorbar()
    cbar2.ax.set_ylabel('$Re_{\lambda_x}$', fontsize=75)
    cbar2.ax.tick_params(labelsize=50)
    plt.xlabel('X',fontsize=75)
    plt.ylabel('Y',fontsize=75)
    plt.xticks(size=50)
