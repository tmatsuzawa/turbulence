...
def save(path, ext='png', dpi=300,close=True, verbose=True):
    """Save a figure from pyplot. -Written by jhamrick
    Parameters
    ----------
    path : string
        The path (and filename, without the extension) where the
...
def save_figs(figs, savedir='./', suffix='', prefix='', frmt='pdf', dpi=300, display=False, data_save=True):
    ...
    dict can be autogenerated from the output of the graphes.legend() function
...
def movie_pivstacks(M, field, indices=None, compute=False, label='', Dirname='./', tracking=False, vmin=0,vmax=10**4, **kwargs):
    """
    Generates png files of heatmap of specified field in specified directory
    ...
    names = ['U_x', 'U_y', 'omega', 'E', 'omega2']
    units = ['$mm/s$', '$mm/s$', 's$^{-1}$', 'mm$^2$/s$^2$', 's$^{-2}$']
    names_latex = ['$U_x$', '$U_y$', '$\\omega_z$', '$E$', '$\\omega^2$']
    units_latex = ['$mm/s$', '$mm/s$', 's$^{-1}$', 'mm$^2$/s$^2$', 's$^{-2}$']
    field = 'E'
    try :
        j = names.index(field)
    except ValueError:
        print field + ' is not possible to plot. Choose from [U_x, U_y, omega, E, omega2] .'
        return None
    ...
    for frame in indices:
        ...
        Mplot(M, 'E', frame, vmin=vmin, vmax=vmax, fignum=1, colorbar=False)
        figs.update(legend('X (mm)', 'Y (mm)', field, cplot=True))

        colorbar(label = names[j] + ' (' + units[j] + ')')

        slicenumber = frame % 17
        Dirname_new = Dirname + 'slice' + str(slicenumber) + '/'
        filepath = Dirname_new + field +'_' + str(frame)
        save(filepath)
