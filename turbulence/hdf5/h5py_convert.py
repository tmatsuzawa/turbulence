...
def create(filename):
    ...
    if not os.path.exists(Dirname):
        print '... The hdf5 file directory does NOT exist. Create one.'
        os.makedirs(Dirname)
    if not os.path.exists(filename):
        print '... The hdf5 file does NOT exist. Create one. The name of the hdf5 file is...'
        print(filename)
...
def open(filename, typ='r'):
    ...
    if os.path.exists(filename):
        ...
        print("File " + filename + " does not exist. Create one.")
        f = h5py.File(filename, 'w')
    return f
    # print('done')
