...
parser.add_argument('-datadir', '--datadir', help='Parent directory of PIVlab output HDF5 directories', type=str,
                    default='/Volumes/labshared3-1/takumi/JHTD-sample/JHT_Database/Data/synthetic_data_from_jhtd/varying_particle_density_unfiltered/hdf5data')
parser.add_argument('-jhtd', '--jhtddatapath', help='full path to jhtd data', type=str,
                    default='/Volumes/labshared3-1/takumi/JHTD-sample/JHT_Database/Data/isotropic1024fine_zl_1_yl_1024_xl_1024_coarse_t0_0_tl_1_y0_0_x0_0_z0_0_.h5')
...
resultdir = os.path.join(parentdir, 'comp_jhtd_pivlab/')
...
fx = 1024 / (2 * np.pi) #px/unit length
...
#     fig2, ax2, cc2 = graph.color_plot(xx, yy, ux, cmap=cmap, vmin=-2, vmax=2, fignum=2,  subplot=(221+i) )
...
#     ux, uy = np.array(data['ux'])[..., 0], np.array(data['uy'])[..., 0]
#     label = '# of particles: %s' % pivlab_output[-8:-3]
#     fig3, ax3, cc3 = graph.color_plot(xx, yy, uy, cmap=cmap, vmin=-2, vmax=2, fignum=3,  subplot=(221+i) )
#     # ax2.quiver(xx, yy, ux, uy)
#     axes3.append(ax3)
#     ccs3.append(cc3)
#     data.close()
...
#     ux, uy = np.array(data['ux'])[..., 0], np.array(data['uy'])[..., 0]
#     label = '# of particles: %s' % pivlab_output[-8:-3]
#     fig4, ax4, cc4 = graph.color_plot(xx, yy, (ux-ux0_coarse.T) / ux0_coarse.T*100, cmap=cmap, vmin=-200, vmax=200, fignum=4, subplot=(221 + i))
#     fig5, ax5, cc5 = graph.color_plot(xx, yy, (uy-uy0_coarse.T) / uy0_coarse.T * 100, cmap=cmap, vmin=-200, vmax=200, fignum=5, subplot=(221 + i))
#     fig13, ax13, cc13 = graph.color_plot(xx, yy, (ux-ux0_coarse.T), cmap=cmap, vmin=-2, vmax=2, fignum=13, subplot=(221 + i))
#     fig14, ax14, cc14 = graph.color_plot(xx, yy, (uy-uy0_coarse.T), cmap=cmap, vmin=-2, vmax=2, fignum=14, subplot=(221 + i))
#     axes4.append(ax4)
#     ccs4.append(cc4)
#     axes5.append(ax5)
#     ccs5.append(cc5)
...
# fig6, ax6 = graph.pdf(ux0, nbins=200, fignum=6, label='JHTD')
# fig7, ax7 = graph.pdf(uy0, nbins=200, fignum=7, label='JHTD')
# linestyles = ['--', '--', '--', '--', '--', '--', '--', '--']
# for i, pivlab_output in enumerate(pivlab_outputs):
#     data = rw.read_hdf5(pivlab_output)
#     xx, yy = np.array(data['x']) * dx, np.array(data['y']) * dy
...
for i, pivlab_output in enumerate(pivlab_outputs):
    ...
    xx, yy = np.array(data['x']) * dx, np.array(data['y']) * dy
    ux, uy = np.array(data['ux'])[..., 0], np.array(data['uy'])[..., 0]
    noise_ux = (ux - ux0_coarse.T) / ux0_coarse.T
    noise_uy = (uy - uy0_coarse.T) / uy0_coarse.T
    label = 'N: %s' % pivlab_output[-8:-3]
    ...
    fig16, ax16, bins, hist = graph.pdf(ux - ux0_coarse.T, nbins=200, label=label, alpha=0.6, fignum=16, return_data=True)
    fig17, ax17, bins, hist = graph.pdf(uy - uy0_coarse.T, nbins=200, label=label, alpha=0.6, fignum=17, return_data=True)
...
#     label = 'N: %s' % pivlab_output[-8:-3]
#
#     fig18, ax18 = graph.scatter(ux.flatten(), (ux - ux0_coarse.T).flatten(), alpha=0.2, fignum=18, subplot=121, figsize=(16, 8))
#     fig18, ax19 = graph.scatter(uy.flatten(), (uy - uy0_coarse.T).flatten(), color='C1', alpha=0.2, fignum=18, subplot=122, figsize=(16, 8))
#     mask1, mask2 = process.get_mask_for_nan_and_inf(ux), process.get_mask_for_nan_and_inf((ux - ux0_coarse.T))
#     mask3, mask4 = process.get_mask_for_nan_and_inf(ux), process.get_mask_for_nan_and_inf((ux - ux0_coarse.T))
#     ux_clean, ux_err_clean = process.delete_masked_elements(ux, mask1), process.delete_masked_elements(
#         (ux - ux0_coarse.T), mask2)
#     uy_clean, uy_err_clean = process.delete_masked_elements(uy, mask3), process.delete_masked_elements(
#         (uy - uy0_coarse.T), mask4)
...
# mask1, mask2 = process.get_mask_for_nan_and_inf(ux), process.get_mask_for_nan_and_inf((ux-ux0_coarse.T))
# mask3, mask4 = process.get_mask_for_nan_and_inf(ux), process.get_mask_for_nan_and_inf((ux-ux0_coarse.T))
# ux_clean, ux_err_clean = process.delete_masked_elements(ux, mask1), process.delete_masked_elements((ux-ux0_coarse.T), mask2)
# uy_clean, uy_err_clean = process.delete_masked_elements(uy, mask3), process.delete_masked_elements((uy-uy0_coarse.T), mask4)
# cov_ux = np.corrcoef([ux_clean, ux_err_clean]) # covariance matrix
# cov_uy = np.corrcoef([uy_clean, uy_err_clean])
# text1 = 'Corr. Coeff.: %.3f' % cov_ux[1, 0]
# text2 = 'Corr. Coeff.: %.3f' % cov_uy[1, 0]
