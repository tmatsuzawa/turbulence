...
data_tracking_basedir_list = [base_dir + 'pistontuning/dx0.0125autophased_polarityfixed2_piston2']
...
parser.add_argument('-fx', '--fx', help='Conversion factor (mm/px) ', type=float, default=1.)
parser.add_argument('-start', '--start', help='frames used to analyze [start, total_frame_number - end]', type=int, default=400)
...
parser.add_argument('-shiftper_enc', '--shiftper_enc', help='shiftper for encoder data', type=float, default=0.2)
...
for data_tracking_basedir in data_tracking_basedir_list:
    ...
    # Specify csv files
    csv_files = glob.glob(data_tracking_basedir + '/20180405_4000fps_span5p0mm_v400mms_f1Hz_*.csv')
    for csv_file in csv_files:
        ...
        y_pos_hdf5 = np.roll(y_pos_hdf5, int(len(y_pos_hdf5) * args.shiftper))
        # plt.plot(time_hdf5, y_pos_hdf5)
        # plt.show()
        ...
        v_hdf5_left_ind, v_hdf5_right_ind = get_velocity_extrema_near_lift(v_hdf5)
        ...
        # shift_arg = y_pos_hdf5.shape[0] - v_hdf5_left_ind + shift_ind


        # #### for sine waves
        # shift_sine_hdf5_arg, ymax_hdf5 = fa.find_max(y_pos_hdf5)
        ...
        shift_arg = y_pos_hdf5.shape[0] - v_hdf5_right_ind + shift_ind

        # #### for sine
        # shift_ind, shift_val = fa.find_nearest(time_hdf5, time_csv[shift_sine_csv_arg])
        # shift_ind = shift_ind + 80
        # shift_arg = y_pos_hdf5.shape[0] - shift_sine_hdf5_arg + shift_ind


        # Match time of the two data curves (tracking code results and software results) at the peak
        y_pos_hdf5 = np.roll(y_pos_hdf5, shift_arg)
        v_hdf5 = np.roll(v_hdf5, shift_arg)
        ...
        short_y_pos_hdf5 = y_pos_hdf5[0:short_time_hdf_max_ind]
        short_pos_csv = pos_csv[0:short_time_hdf_max_ind]
        stroke_length_hdf5 = np.max(short_y_pos_hdf5)-np.min(short_y_pos_hdf5)
        ...
        time_enc, pos_enc = fa.sort_two_arrays_using_order_of_first_array(time_enc, pos_enc)
        #print time_enc, pos_enc
        # Roll pos_enc
        print int(len(pos_enc) * args.shiftper_enc), len(pos_enc), args.shiftper_enc
        pos_enc_new = np.roll(pos_enc, int(len(pos_enc) * args.shiftper_enc))
        fig2, ax21 = graph.plot(time_enc, pos_enc, fignum=2, subplot=221, figsize=(8, 16))
        time_v, vel = process.compute_velocity_simple(time_enc, pos_enc)
        fig2, ax22 = graph.plot(time_enc, vel, fignum=2, subplot=222, figsize=(8, 16))


        # get data averaged over periods
        time_enc_short, pos_enc_short_avg, pos_enc_short_std = fa.get_average_data_from_periodic_data(time_enc, pos_enc, freq=freq)
        time_enc_short_v, vel_enc_short_avg, vel_enc_short_std,\
        time_chunks, vel_chunks, time_chunks_int, vel_chunks_int = fa.get_average_data_from_periodic_data(time_enc, vel, freq=freq, returnChunks=True)

        #graph.scatter(time_enc, pos_enc_new, fignum=2, subplot=211)
        pos_enc_short_avg = np.roll(pos_enc_short_avg, int(len(pos_enc_short_avg) * args.shiftper_enc))
        pos_enc_short_std = np.roll(pos_enc_short_std, int(len(pos_enc_short_std) * args.shiftper_enc))
        fig2, ax23, color_patch23 = graph.errorfill(time_enc_short, pos_enc_short_avg, pos_enc_short_std, fignum=2, subplot=223, figsize=(16, 16))
        print np.max(pos_enc_short_avg)-np.min(pos_enc_short_avg)

        vel_enc_short_avg = np.roll(vel_enc_short_avg, int(len(vel_enc_short_avg) * args.shiftper_enc))
        vel_enc_short_std = np.roll(vel_enc_short_std, int(len(vel_enc_short_std) * args.shiftper_enc))
        fig2, ax24, color_patch24= graph.errorfill(time_enc_short_v, vel_enc_short_avg, vel_enc_short_std, fignum=2, subplot=224,  figsize=(16, 16))
        ## Plot curves for each period
        # for i in range(len(vel_chunks)):
        #     time_chunks[i] = time_chunks[i] - np.min(time_chunks[i])
        #     time_chunks_int[i] = time_chunks_int[i] - np.min(time_chunks_int[i])
        #     vel_chunks_int[i] = np.roll(vel_chunks_int[i], int(len(vel_chunks_int[i]) * args.shiftper_enc))
        #     graph.plot(time_chunks_int[i], vel_chunks_int[i], fignum=2, subplot=224, figsize=(8, 16), linestyle=':')

        # Make plot presentable
        graph.labelaxes(ax21, 'Time (s)', 'Position (mm)')
        graph.labelaxes(ax22, 'Time (s)', 'Velocity (mm/s)')
        graph.labelaxes(ax23, 'Time (s)', 'Position (mm)')
        graph.labelaxes(ax24, 'Time (s)', 'Velocity (mm/s')
        ax21.set_title('Position vs Time')
        ax22.set_title('Velocity vs Time')
