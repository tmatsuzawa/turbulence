...
def save_figs(figs, savedir='./', suffix='', prefix='', frmt='pdf', dpi=300, display=False, data_save=True):
    """Save a dictionary of labeled figures using dictionnary elements
    dict can be autogenerated from the output of the graphes.legende() function
    by default the figures are saved within the same folder from where the python code has been called
...
def save_fig(fignum, filename, frmt='pdf', dpi=300, overwrite=False, display=False):
    """
...
def Mplot(M, field, frame, auto_axis=False, step=1, W=None, Dt=None, fignum=1, show=False, vmin=0, vmax=0, log=False,
    ...
    ----------
    M : Mdata class oject
        Mdata object contains Ux, Uy, fx, etc. Check /mdata/Mdata.py for all attributes.
    field : str
        Variable you want to plot. Choices are... E, enstrophy, Ux, Uy, omega, strain
    frame : int
        frame number
    auto_axis : bool, deafult: False
        If True, it tries to autoset vmin and vmax.
    step : int, default=1
        ?
    W : ?
    Dt : ?
...
def pdf(M, field, frame, Dt=10, Dx=1024, label='ko-', fignum=1, a=15., norm=True, sign=1):
    ...
    figs = distribution(Up, normfactor=normfactor, a=a, label=label, fignum=fignum, norm=norm)
...
def distribution(Y, normfactor=1, a=10., label='k', fignum=1, norm=True):
    """
    Generates a probability distribution function (if norm==True) or a histogram (if norm==False)
    Parameters:
    :param Y: numpy array
    :param normfactor:
    :param a:
    ...
    """
    #norm is a horrible name for this boolean. It is NOT a boolean to normalize the pdf or not,
    Y = np.asarray(Y)
